...
import models.{ClusterJob, ClusterJobCompleted, GetAllInvoicesResponse, GetInvoiceResponse, Invoice}
...
object AkkaHttpClusterExample extends  App with JsonSupport {
  ...
  val configPath = if (args.length == 0) "http_cluster1" else args(0)
